...
| `useStations` | `useStations.ts` | `{ stations: GasStation[], statusCounts: Record<StationStatus, number>, loading: boolean, error: boolean }` | None (one-time fetch) | Fetches `public/data/stations.json` once on mount via a module-level singleton promise. Assigns a deterministic status to each station (`assignStationStatus`) and tallies `statusCounts`. Returns empty data while loading or on error. |
