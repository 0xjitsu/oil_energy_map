...
| Component | File | Key Props | Used By | Notes |
|-----------|------|-----------|---------|-------|
| `IntelMap` | `map/IntelMap.tsx` | `mapMode`, `scenarioParams`, `timelinePosition`, `onModeChange` | Dashboard page (page.tsx) | Core map — MapLibre GL + deck.gl via `DeckGLOverlay`. Default-exported; `page.tsx` imports it directly via `next/dynamic` (`ssr: false`). Manages all layer/filter state; fetches the station dataset via `useStations`. Default center: Philippines (122, 12, zoom 5.5). |
...
| `StationLayer` | `map/StationLayer.tsx` | — (factory function) | `IntelMap` | Factory: `createStationLayer(filteredStations, visible, onSelect, hoveredId, setHoveredId, onHoverInfo, zoom, statusFilter, cacheKey, cache)` → `Layer[]`. Receives stations already filtered by `filterStations`; clusters via `supercluster` at zoom < 8 using an instance-scoped `ClusterCache`. |
...
| `SparkChart` | `prices/SparkChart.tsx` | `data: number[]`, `color: string`, `width?: number`, `height?: number`, `unit?: string` | `PricePanel`, `ExecutiveSnapshot`, `PumpPrices`, `SentimentGauge` | Hand-rolled SVG sparkline — gradient area fill + trend line + last-point dot. Default 80×24px. No Recharts dependency; `<title>` + `aria-label` for accessibility. Renders a fixed-size empty box when given fewer than 2 points. |
...
| `StationTrackerSection` | `health/StationTrackerSection.tsx` | — | Dashboard page | 5 stat cards for station status counts (total, out-of-stock, low-supply, closed, operational) from `useStations` (`statusCounts` + `stations.length`). Dataset: 10,469 stations. |
...
| `useEvents` | `useEvents.ts` | `{ events: TimelineEvent[], isLive: boolean, lastUpdated: Date \| null }` | 3 min (in `DataProvider`) | Thin reader over the `DataProvider` context — does not poll itself. Polling, 3× exponential-backoff retry, and `@/data/events` static fallback all live in `DataProvider`. Source: `/api/events` (RSS aggregator). |
...
| `usePrices` | `usePrices.ts` | `{ prices: PriceBenchmark[], isLive: boolean, lastUpdated: Date \| null, priceHistory: Record<string, number[]> }` | 5 min (in `DataProvider`) | Thin reader over the `DataProvider` context — does not poll itself. Polling, 7-reading history accumulation (~35 min of sparkline points), and `@/data/prices` static fallback all live in `DataProvider`. Source: `/api/prices` (Yahoo Finance + FloatRates + DOE). |
...
| `useStations` | `useStations.ts` | `{ stations: GasStation[], statusCounts: Record<StationStatus, number>, loading: boolean, error: boolean }` | None (one-time fetch) | Fetches the content-hashed `public/data/stations/<hash>.json` (path from the committed `src/data/stations-manifest.json`, served with immutable Cache-Control via `vercel.json`) once on mount via a module-level singleton promise. Assigns a deterministic status to each station (`assignStationStatus`) and tallies `statusCounts`. Returns empty data while loading or on error. |
